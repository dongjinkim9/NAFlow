...
import archs.flow_modules
import archs.flow_modules.Permutations
from archs.flow_modules import flow, thops, FlowAffineCouplings
from archs.flow_modules.flow import BentIdentPar
from utils.common import opt_get


def getConditional(condResults, position):
    img_ft = condResults if isinstance(condResults, torch.Tensor) else condResults[position]
    return img_ft
...
class FlowStep(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels,
        ...
        if self.norm_type == "ConditionalActNormImageInjector":
            ...
            self.actnorm = archs.flow_modules.FlowActNorms.ConditionalActNormImageInjector(in_channels=64,
            ...
            self.actnorm = archs.flow_modules.FlowActNorms.ActNorm2d(in_channels, actnorm_scale)

        # 2. permute
        if flow_permutation == "invconv":
            min_singular = opt_get(opt, ["network_G", "flow", "InvertibleConv1x1", "min_singular"], None)
            self.invconv = archs.flow_modules.Permutations.InvertibleConv1x1(
            ...
            self.invconv = archs.flow_modules.Permutations.SqueezeInvertibleConv1x1(in_channels,
                                                                                LU_decomposed=LU_decomposed,
                                                                                squeezeFactor=squeezeFactor)
        elif flow_permutation == "resqueeze_invconv_alternating_2_3":
            if idx % 2 == 0:
                self.invconv = archs.flow_modules.Permutations.InvertibleConv1x1(in_channels, LU_decomposed=LU_decomposed)
            else:
                self.invconv = archs.flow_modules.Permutations.InvertibleConv1x1Resqueeze(in_channels,
                                                                                      LU_decomposed=LU_decomposed,
                                                                                      resqueeze=3)
            ...
            self.invconv = archs.flow_modules.Permutations.InvertibleConv1x1GridAlign(in_channels)
            ...
            self.invconv = archs.flow_modules.Permutations.InvertibleConv1x1SubblocksShuf(in_channels,
                                                                                      LU_decomposed=LU_decomposed,
                                                                                      n_blocks=n_blocks)
            ...
            if idx % 2 == 0:
                ...
                self.invconv = archs.flow_modules.Permutations.InvertibleConv1x1GridAlignIndepBorder(in_channels)
            ...
            self.shuffle = archs.flow_modules.Permutations.Permute2d(in_channels, mode="shuffle")
        elif flow_permutation == "RandomRotation":
            
            self.invconv = archs.flow_modules.Permutations.RandomRotation(in_channels)
        
        else:
            self.reverse = archs.flow_modules.Permutations.Permute2d(in_channels, shuffle=False)
        ...
        if flow_coupling == "additive":
            ...
            self.affine = archs.flow_modules.FlowAffineCouplings.AffineCoupling(acOpt=acOpt)
        elif flow_coupling == "condAffine":
            self.affine = archs.flow_modules.FlowAffineCouplings.CondAffineCoupling(acOpt=acOpt, in_channels=in_channels)
        elif flow_coupling == "condFtAffine":
            self.affine = archs.flow_modules.FlowAffineCouplings.CondFtAffineCoupling(acOpt=acOpt, in_channels=in_channels)
        elif flow_coupling == "condNormAffine":
            self.affine = archs.flow_modules.FlowAffineCouplings.CondNormAffineCoupling(acOpt=acOpt,
                                                                                    in_channels=in_channels)
        elif "CondFea" in flow_coupling and "AndCondSelf" in flow_coupling:
            mode = flow_coupling.replace("CondFea", "").replace("AndCondSelf", "_")
            self.affine = archs.flow_modules.FlowAffineCouplings.CondFeaAndCondSelf(in_channels=in_channels, mode=mode, opt=opt)
        elif "CondFeaAffine" in flow_coupling:
            mode = flow_coupling.replace("CondFeaAffine", "")
            self.affine = archs.flow_modules.FlowAffineCouplings.CondFeaAffine(in_channels=in_channels, mode=mode, opt=opt)
        elif "CondSelfAffine" in flow_coupling:
            mode = flow_coupling.replace("CondSelfAffine", "")
            self.affine = archs.flow_modules.FlowAffineCouplings.CondSelfAffine(in_channels=in_channels, mode=mode, opt=opt)
        else:
            raise RuntimeError("coupling not Found")

    def forward(self, input, logdet=None, reverse=False, condResults=None):
        if not reverse:
            return self.normal_flow(input, logdet, condResults)
        else:
            return self.reverse_flow(input, logdet, condResults)

    def normal_flow(self, z, logdet, condResults=None):
        if self.flow_coupling == "bentIdentityPreAct":
            z, logdet = self.bentIdentPar(z, logdet, reverse=False)

        # 1. actnorm
        if self.norm_type == "ConditionalActNormImageInjector":
            img_ft = getConditional(condResults, self.position)
        ...
        if self.flow_coupling == "additive":
            ...
            z = thops.cat_feature(z1, z2)
        elif need_features or self.flow_coupling in ["cond", "condAffine", "condFtAffine", "condNormAffine"]:
            img_ft = getConditional(condResults, self.position)
    ...
    def reverse_flow(self, z, logdet, condResults=None):
